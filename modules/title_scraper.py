...
class TitleScraper(NetworkModule):
    ...
    async def get_titles_by_creator(self, session: aiohttp.ClientSession, creator_id: str, page: int) -> List[Tuple[int, str, str, int, str]]:
        ...
        if response_text:
            ...
            for idx, result in enumerate(results, start=1):
                ...
                if thumb_frame:
                    ...
                    if meta_tag:
                        title = meta_tag.get('content', '').strip()
                        titles.append((timestamp, creator_id, page, idx, title))
        
        return titles

    async def scrape_titles_by_creator(self, creator_ids: List[str], pages: List[int]) -> List[Tuple[int, str, str, int, str]]:
    ...
    def scrape_titles_by_creator_to_csv(self, creator_ids_string: str, pages_string: str) -> str:
        ...
        headers = ['Timestamp', 'Creator ID', 'Page', 'Result Number', 'Title']
